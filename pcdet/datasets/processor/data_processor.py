...
class VoxelGeneratorWrapper():
    """Wrapper for VoxelGenerator that supports different spconv versions."""
...
class DataProcessor:
    """
    Enhanced data processor that handles point cloud processing operations.
    """
    ...
    def mask_points_and_boxes_outside_range(self, data_dict=None, config=None):
        """Mask points and boxes outside the point cloud range."""
    ...
    def transform_points_to_voxels(self, data_dict=None, config=None):
        """Transform point cloud to voxels."""
        if data_dict is None:
        ...
        if config.get('DOUBLE_FLIP', False):
            ...
            for i, key in enumerate(['yflip', 'xflip', 'xyflip']):
            ...
            data_dict.update({
                'voxels': voxels_list,
                'voxel_coords': voxel_coords_list,
    ...
    def double_flip(self, points):
        """Perform double flip augmentation on points."""
    ...
    def image_calibrate(self, data_dict=None, config=None):
        ...
        for img_process_info in img_process_infos:
            ...
            transform_matrix = self._compute_image_transform(resize, crop, flip, rotate)
            transforms.append(transform_matrix)

        data_dict["img_aug_matrix"] = transforms
        return data_dict

    def _compute_image_transform(self, resize, crop, flip, rotate):
        """Helper method to compute image transformation matrix."""
    ...
    def forward(self, data_dict: Dict) -> Dict:
        """
        Process the data dictionary through all processors in the queue.
        
        Args:
            data_dict: Dictionary containing point cloud and annotation data
                points: (N, 3 + C_in)
        ...
        """
        for processor in self.data_processor_queue:
